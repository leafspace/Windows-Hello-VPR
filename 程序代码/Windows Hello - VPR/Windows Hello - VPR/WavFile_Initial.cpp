...
bool WavFile_Initial::Frame_Energy(void)                                     //用于求短时帧能量
{
	...
	double sum = 0;
	for (unsigned long i = 0; i < WavFile::Get_dataNumber() - N; i += WavFile_Initial::FrameShift){        //这是所有短时帧能量数据的个数
...
bool WavFile_Initial::Frame_ZCR(void)                                        //用于求短时过零率
{
	...
	double sum = 0;
	for (unsigned long i = 0; i < WavFile::Get_dataNumber() - N; i += WavFile_Initial::FrameShift) {       //这是所有短时帧过零率数据的个数
		for (unsigned long j = i; j < i + N; ++j) {                          //遍历窗中的每一个数据
...
{
	if (Number >= (WavFile::Get_dataNumber()) || Number < 0) {               //如果所需要的数超过了数据个数
...
bool WavFile_Initial::Endpoint_Detection(void)                               //端点检测函数
{
	...
	for (unsigned long i = 0, frame = 0; i < WavFile::Get_dataNumber() - N; ++i) { //遍历每一帧
